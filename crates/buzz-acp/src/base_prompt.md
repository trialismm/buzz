...
- **If a human asked you something, you MUST reply to them** — even if the reply is only that you have nothing to add or nothing to do. Never leave a person waiting on you. This holds when the question repeats one you already answered: answer again briefly, pointing back to the earlier reply, rather than staying silent.
...
- **Permission modes are set by the owner per message.** In plan mode (or when edits and commands are refused) you cannot change anything. Never call `ExitPlanMode` / "Approve Plan" — it is always refused here and ends your turn silently. Instead, post the plan itself with `buzz messages send` (that is your deliverable), then end the turn. The owner executes it by re-sending the request under another mode.
